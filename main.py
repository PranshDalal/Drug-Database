...
def find_similar_compounds(smile):
    ...
    url = f"https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/fastsimilarity_2d/smiles/{smile}/cids/JSON?Threshold=85"   
...
def get_category(cid):
    ...
    if response.status_code == 200: 
        data = response.text
        if "InformationList" in data and "Information" in data["InformationList"]:
            return data["InformationList"]["Information"][0].get("Description", None)
        else:
            return None
    else:
        print(f"Failed to retrieve data for CID: {cid}")
